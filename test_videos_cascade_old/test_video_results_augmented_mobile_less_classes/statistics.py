...
def main():
    ...
    # less_classes_matrix = None
    # for video_id in video_ids:
    #     print("Confusion Matrix for Video: ", video_id)
    video_id = "GX010093"

    actual_file = fr"C:\Users\aaron\Desktop\Programmierung\Master\Machine Vision\Computer-Robot_Vision_repo\test_video_results_augmented_efficientNet\tests\{video_id}\{video_id}_labels.xlsx"
    predicted_file = fr"C:\Users\aaron\Desktop\Programmierung\Master\Machine Vision\Computer-Robot_Vision_repo\test_video_results_augmented_efficientNet\tests\{video_id}\predicted_labels_{video_id}.csv"
    actual_labels = read_labels(actual_file)
    predicted_labels = read_labels(predicted_file, False)
    efficient_matrix = create_confusion_matrix(actual_labels, predicted_labels, class_labels_less_classes)

    efficient_confusion_matrix_path = fr"C:\Users\aaron\Desktop\Programmierung\Master\Machine Vision\Computer-Robot_Vision_repo\test_video_results_augmented_efficientNet\tests\{video_id}\{video_id}_confusion_matrix.npy"
    np.save(efficient_confusion_matrix_path, efficient_matrix)

    # display_matrix(less_classes_matrix, class_labels_less_classes, "GX010093", False, True)
    # all_classes_confusion_matrix_path = fr"C:\Users\aaron\Desktop\Programmierung\Master\Machine Vision\Computer-Robot_Vision_repo\test_video_results_augmented_mobile\tests\{video_id}\{video_id}_confusion_matrix.npy"
    # less_classes_confusion_matrix_path = fr"C:\Users\aaron\Desktop\Programmierung\Master\Machine Vision\Computer-Robot_Vision_repo\test_video_results_augmented_mobile_less_classes\tests\{video_id}\{video_id}_confusion_matrix.npy"
    # efficient_confusion_matrix_path = fr"C:\Users\aaron\Desktop\Programmierung\Master\Machine Vision\Computer-Robot_Vision_repo\test_video_results_augmented_efficientNet\tests\{video_id}\{video_id}_confusion_matrix.npy"


    # all_classes_matrix = np.load(all_classes_confusion_matrix_path)
    # less_classes_matrix = np.load(less_classes_confusion_matrix_path)
    # efficient_matrix = np.load(efficient_confusion_matrix_path)

    # plt.figure(figsize=(10, 8))
    # display_matrix(less_classes_matrix, class_labels_less_classes, video_id+ "Less Classes", False, False)
    # plt.figure(figsize=(10, 8))
    # display_matrix(all_classes_matrix, class_labels, video_id + " All Classes", False, False)
    # plt.figure(figsize=(10, 8))
    # display_matrix(efficient_matrix, class_labels_less_classes, video_id + " Efficient", False, False)
    # # Zeige alle Fenster an
    # plt.show()
